/*CONTROL SYSTEM FOR STEPPER MOTORS ATTACHED TO SHAKER 1
  ...
  1) To gain a 2x speed up of the microstepping edit the header file when building to
  #define MICROSTEPS 8 // 8 or 16.

  2) There is a quirk in the design. Motor2 seemed to max out at 1000 steps and so if you want more the 
  code splits it up into 1000 step chunks

  3) Seems like there is some backlash in the system. To minimise this each motor is designed to always 
  finish pushing the weight of the shaker upwards. If you request a downwards move this is interpreted as move the
  distance downwards requested. Then move down nsteps more. Then move nsteps up. If request upwards move it just
  moves as instructed.
...
const int BACKLASH_CORRECTION = 100;                                //number steps to move past and then back on downwards moves.
        ...
        switch (data[index + 1]){
          case '1':
            ...
            if (direction == BACKWARD){
              //Overshoots and returns on downwards movements to correct backlash
              stepper1->step(BACKLASH_CORRECTION, BACKWARD, MICROSTEP); // MICROSTEP
              stepper1->step(BACKLASH_CORRECTION, FORWARD, MICROSTEP); // MICROSTEP
            }
            stepper1->release();
          ...
          case '2':
            ...
            if (direction == BACKWARD){
              //Overshoots and returns on downwards movements to correct backlash
              stepper2->step(BACKLASH_CORRECTION, BACKWARD, MICROSTEP); // MICROSTEP
              stepper2->step(BACKLASH_CORRECTION, FORWARD, MICROSTEP); // MICROSTEP
            }
            stepper2->release();
