...
def balance_shaker(initial_iterations=10, ncalls=20, boundary_pts=None, com_func=None):
    ...
    with Shaker() as shaker, StepperXY() as motors:
        ...
        result = bal.level(com_func, dimensions=dimensions, use_pts=False,
                           initial_iterations=initial_iterations, ncalls=ncalls, tolerance=2)
        # move motors to optimal positions.
        motors.movexy(result.x[0], result.x[1])
